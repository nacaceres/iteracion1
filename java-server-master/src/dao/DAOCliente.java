...
public class DAOCliente {
	...
	public ArrayList<UsoAlohAndes> getUsoAlohAndes() throws SQLException, Exception {
		...
		String sql = "SELECT RELA.TIPO,ALO.TIPO AS TIPO_ALOJAMIENTO,SUM(RE.COSTO_DEFINITIVO) AS DINERO_PAGADO,SUM(RE.NUM_DIAS) AS DIAS_CONTRATADOS FROM "+USUARIO+".CLIENTES CLI INNER JOIN "+USUARIO+".RESERVAS RE ON RE.ID_CLIENTE=CLI.ID INNER JOIN "+USUARIO+".ALOJAMIENTOS ALO ON ALO.ID=RE.ID_ALOJAMIENTO INNER JOIN "+USUARIO+".RELACIONES RELA ON RELA.ID=CLI.ID_RELACION GROUP BY RELA.TIPO,ALO.TIPO";
